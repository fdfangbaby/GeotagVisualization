...
//			  });
	...
	function showForceGraph()
	{
		...
		var svg_texts = svg.selectAll("text")
			...
			.attr("dy", 8)
			.text(function(d) { 
			if(d.name!="速度") return d.name+":"+d.value; 
			return d.name; 
			});
		...
		svg.append("g")
			...
			.on("mouseover",function(d,i){
					...
					if(data[d.source.index][d.target.index]!=0)
						d3.select("#textForSimilarity").text("相似度："+new Number(1/data[d.source.index][d.target.index]).toFixed(4));
					else d3.select("#textForSimilarity").text("相似度："+new Number(data[d.source.index][d.target.index]).toFixed(4));
			})
	...
	function showStatistical(path){
		var district = ["黄浦","静安","徐汇", "浦东", "虹口","杨浦", "宝山", "普陀", "长宁", "闵行"];
...
//	});
		...
		testData.taxiId = taxiId;
		 ...
		 $.ajax({ 
	            ...
	            success:function(data){ 
            		...
            		nodes.push(new Node("速度",null));
            		nodes.push(new Node("总行程数",pathCount+"次"));
            		nodes.push(new Node("地理数据点总数",data.length));
            		nodes.push(new Node("行驶总距离",new Number(totalDistance).toFixed(2)+"km"));
            		
            		nodes.push(new Node("最高时速",maxSpeed+"km/h"));
            		nodes.push(new Node("最低时速",minSpeed+"km/h"));
            		nodes.push(new Node("平均时速",parseInt(totalSpeed/data.length)+"km/h"));
            		nodes.push(new Node("空载",noLoadCount+"次"));
            		nodes.push(new Node("载客",onLoadCount+"次"));
            		nodes.push(new Node("总用时",new Number(timeCompare(data[data.length-1].time,data[0].time)).toFixed(2)+"min"));
            		nodes.push(new Node("最长行驶旅程",new Number(longestDistance).toFixed(2)+"km"));
            		nodes.push(new Node("最短行驶旅程",new Number(shortestDistance).toFixed(2)+"km"));
	...
	$("#show_single_data_previous_page").click(function(){
		if(singlePathIndex==0) {alert("这是第一页");return;}
		$(".demo--radio").attr("checked",false);
		singlePathIndex--;
		testData.index = singlePathIndex;
		$.ajax({
	...
	$("#show_compared_data_previous_page").click(function(){
		if(comparedPathIndex==0) {alert("这是第一页");return;}
		$(".demo--radio").attr("checked",false);
		comparedPathIndex--;
		testData.index = comparedPathIndex;
		$.ajax({
